            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="roomModals-body">
                        <div class="content-item">
                            ...
                            <div class="userInfo-items">
                                <div class="userInfo-item">
                                    ...
                                    <input class="dd-input" type="text" maxlength="16" placeholder="联系人姓名" id="name" autocomplete="off" :disabled="this.checkState === 'editOrder' && !(order.type === ORDER_TYPE.COMBINATION || (order.type === ORDER_TYPE.ACCOMMODATION && !order.isCombinationOrder))" v-model="name" @input="changeVipList(1)">
                                </div>
                                <div class="userInfo-item userInfo-phone vip-level-container">
                                    <label for="phone">手机号</label>
                                    <input class="dd-input" type="text" id="phone" maxlength="20" placeholder="手机号" autocomplete="off" :disabled="this.checkState === 'editOrder' && !(order.type === ORDER_TYPE.COMBINATION || (order.type === ORDER_TYPE.ACCOMMODATION && !order.isCombinationOrder))" v-model="phone" @input="changeVipList(2)">
                        ...
                        <EnterEditor :order="order" v-if="this.checkState !== 'editOrder' || (order.type === ORDER_TYPE.ENTERTAINMENT || order.type === ORDER_TYPE.COMBINATION || (order.type === ORDER_TYPE.ACCOMMODATION && !order.isCombinationOrder))" :vipDiscountDetail="vipDiscountDetail" @change="handleEnterChange" @priceChange="handlEnterPriceChange" />
                        <ShopEditor v-show="this.checkState !== 'editOrder' || (order.type === ORDER_TYPE.RETAIL || order.type === ORDER_TYPE.COMBINATION || (order.type === ORDER_TYPE.ACCOMMODATION && !order.isCombinationOrder))" :order="order" :vipDiscountDetail="vipDiscountDetail" @change="handleShopChange" @priceChange="handleShopPriceChange">
                        </ShopEditor>
    ...
    methods: {
        ...
        getSubmitRooms() {
            return this.rooms.map(room => {
                return {
                    ...
                    fee: room.price,
                    sub: true,
                    checkRoomTypeErr: room.checkRoomTypeErr,
        ...
        handleQuickTeamBusiness(type) {
            ...
            this.rooms.forEach(function(el) {
                    if (el.checkRoomTypeErr) {
                        return;
                    }
                    if (el.endDate < new Date()) {
                        modal.confirm({
                                title: '提示',
                                message: '正常入住[大床房201]已超时，确定保存订单吗？'}, () => {callBack(params); });
                            }
                        })
