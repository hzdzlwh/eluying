...
export default {
    ...
    methods: {
        ...
        changeSeat() {
            const params = { orderId: this.restOrder.orderId, caterOrderId: this.restOrder.caterOrderId, restId: this.restId };
            params.boardIds = [];
            this.tableList.forEach(area => {
                area.boardList.forEach(board => {
                    if (board.selected) {
        ...
        selectSeat(board) {
            if (this.restOrder.orderState === 0) {    // 预约订单 可以选所有状态桌子
            ...
            return new Promise((resolve, reject) => {
                http.get('/board/list', { date: this.date, restId: this.restId, isChange: true }).then(res => {
                    if (res.code === 1) {
                        ...
                        if (this.restOrder) {
                            this.restOrder.boardDetailResps.forEach(el => {
    ...
    watch: {
        ...
        restOrder(newValue) {
            ...
            this.date = newValue.creationTime.substring(0, 10);
            this.getSeatList();
        }
    },
    components: {
